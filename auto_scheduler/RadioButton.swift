...
class SSRadioButton: UIButton {
    ...
    fileprivate func circleFrame() -> CGRect {
        var circleFrame = CGRect(x: 10, y: 0, width: 2*circleRadius, height: 2*circleRadius)
        circleFrame.origin.x = 0 + circleLayer.lineWidth
        circleFrame.origin.y = bounds.height/2 - circleFrame.height/2
        return circleFrame
    }
    
    convenience init(withValue color: UIColor, frame: CGRect) {
        self.init(frame: frame)
        self.InitLabel(color: color)
    }
    ...
    func InitLabel(color: UIColor, radioCircleColor: UIColor = UIColor.purple){
        self.layer.borderWidth = 2.0;
        self.layer.cornerRadius = 8;
        self.backgroundColor = color;
        self.layer.masksToBounds = true;
        self.circleColor = radioCircleColor;
    }
